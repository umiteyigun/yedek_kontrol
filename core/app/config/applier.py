...
def _render_instance_shell(inst: InstanceSettings, settings: YedekSettings) -> str:
    ...
    lines = [
        ...
        "# ORACLE_HOME: oratab SID eslesmesi, sonra disk (db / db_1)",
        'if [[ -z "${ORACLE_HOME:-}" && -n "$ORACLE_SID" && -f /etc/oratab ]]; then',
        '  ORACLE_HOME="$(awk -F: -v sid="$ORACLE_SID" \'tolower($1)==tolower(sid) && $0 !~ /^#/ { print $2; exit }\' /etc/oratab)"',
        "fi",
